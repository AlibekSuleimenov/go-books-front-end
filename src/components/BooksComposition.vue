                    ...
                    <transition-group class="p-3 d-flex flex-wrap" tag="div" appear name="books">
                        <div v-for="book in books" :key="book.id">
                            ...
                            <router-link :to="`/books/${book.slug}`">
                                <img :src="`${imagePath}/covers/${book.slug}.jpg`" class="card-img-top"
...
export default {
    ...
    setup(props, ctx) {
        ...
        const imagePath = ref(process.env.VUE_APP_IMAGE_URL);
        let books = ref({});

        // use onMounted lifecycle hook to get all books
        onMounted(() => {
